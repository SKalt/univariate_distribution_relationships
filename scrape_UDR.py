...
if __name__ == "__main__":
    ...
    for dist in distributions:
        ...
        if href in nodes:
            ...
            coords = [int(i) for i in dist.get("coords").split(',')]
            nodes[href]["original_coords"] = coords
        else:
            nodes[href] = {
    ...
    with open("./data/original_UDR_graph.json", "w") as json_file:
        json_file.write(json.dumps(d3_json))

    # produce nodelist and edgelist CSVs
    nodelist = pd.DataFrame.from_dict(nodes, orient="index")
    nodelist.to_csv('./data/original_UDR_graph_nodelist.csv', index=False)
    edgelist = pd.DataFrame.from_dict(edges, orient="index")
    edgelist.to_csv('./data/original_UDR_graph_edgelist.csv', index=False)
